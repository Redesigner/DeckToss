...
void UBaseAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetDamageAttribute())
	{
		...
		if (Data.EvaluatedData.Magnitude > 0.0f)
		{
			...
			{
				UE_LOGFMT(LogDeckGame, Display, "'{OwnerName}' took '{Value}' self inflicted damage", GetOwningActor() ? GetOwningActor()->GetName() : "Null", Data.EvaluatedData.Magnitude);
			}
		}
	...
	if (Data.EvaluatedData.Attribute == GetDamageAttribute() || Data.EvaluatedData.Attribute == GetHealthAttribute())
	{
		if (GetHealth() <= 0.0f && GetHealth() != HealthBeforeAttributeChange)
		{
			...
			Payload.EventTag = DeckGameplayTags::GameplayEvent_Death;

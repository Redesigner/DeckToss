...
TArray<FGameplayEffectSpecHandle> FGameplayEffectContainer::MakeEffectSpecs(UGameplayAbility* OwningAbility) const
{
	...
	if (!OwningAbility)
	{
		UE_LOGFMT(LogDeckGame, Error, "GameplayEffectContainer failed to generate EffectSpecs. The owning ability was null.");
	...
	for (FGameplayEffectMagnitudeEntry MagnitudeEntry : Effects)
	{
		...
		{
			UE_LOGFMT(LogDeckGame, Error, "GameplayEffectContainer failed to create a spec for GameplayAbility '{AbilityName}'. The EffectClass was invalid.", OwningAbility->GetName());
			continue;
		}
		...
		{
			UE_LOGFMT(LogDeckGame, Error, "GameplayEffectContainer failed to create spec of GameplayEffect '{GameplayEffectName}'.", MagnitudeEntry.Effect->GetName());
			continue;
		}
